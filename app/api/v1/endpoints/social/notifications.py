...
async def get_notification_details(
    ...
    try:
        logger.info(f"Fetching details for notification {notification_id} for user {current_user.id}")
        ...
        if not notif:
            ...
            global_check = await get_collection("notifications").find_one({"_id": ObjectId(notification_id)})
            if global_check:
                debug_msg = f"Notification exists but belongs to user {global_check['user_id']} (Current: {current_user.id})"
                logger.warning(debug_msg)
                raise HTTPException(status_code=404, detail=debug_msg)
            else:
                logger.warning("Notification does not exist at all in database")
                raise HTTPException(status_code=404, detail="Notification does not exist at all in database")
